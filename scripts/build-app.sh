...
cd "$PROJECT_DIR"
/usr/bin/xcrun swift build -c release --arch arm64 --scratch-path "$ARM64_SCRATCH"
/usr/bin/xcrun swift build -c release --arch x86_64 --scratch-path "$X86_64_SCRATCH"
ARM64_BIN_DIR="$(/usr/bin/xcrun swift build -c release --arch arm64 --scratch-path "$ARM64_SCRATCH" --show-bin-path)"
X86_64_BIN_DIR="$(/usr/bin/xcrun swift build -c release --arch x86_64 --scratch-path "$X86_64_SCRATCH" --show-bin-path)"
